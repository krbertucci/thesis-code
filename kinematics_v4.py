...
def append_angles (angle_arr, angle_data,value):
    '''Args: 
    angle_arr = name of specific angle, containing appended or empty vals
    angle_data = calculated joint angles following trial processing
    value = 'min', 'max', or 'mean' '''
    if value == 'min':
        angle_value = np.min(angle_data)
    elif value == 'mean':
        angle_value = np.mean(angle_data)
    elif value == 'max':
        angle_value = np.max(angle_data)
    else:
