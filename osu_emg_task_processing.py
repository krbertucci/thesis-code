...
trialnum = [0,6] # number of trials 

for muscle, col in muscles.items(): 
    # set easy difficulty paths
    eh_trial_path = f"{subject_folder}\{sub_num}_EASY_HIGH_{trialnum}_a.tsv"
    el_trial_path = f"{subject_folder}\{sub_num}_EASY_LOW_{trialnum}_a.tsv"
    ep_trial_path = f"{subject_folder}\{sub_num}_EASY_PREF_{trialnum}_a.tsv"

    # set hard difficulty paths
    hh_trial_path = f"{subject_folder}\{sub_num}_HARD_HIGH_{trialnum}_a.tsv"
    hl_trial_path = f"{subject_folder}\{sub_num}_HARD_LOW_{trialnum}_a.tsv"
    hp_trial_path = f"{subject_folder}\{sub_num}_HARD_PREF_{trialnum}_a.tsv"
